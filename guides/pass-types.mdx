...
| `barcodeFormat` | No | Barcode format for the pass code - `QR` (default), `PDF417`, `Aztec`, or `Code128` |
...
See the [image dimensions guide](/guides/image-dimensions) for recommended sizes for `icon`, `logo`, `banner`, and `thumbnail`, and the [barcode types guide](/guides/barcode-types) for choosing a `barcodeFormat` and its code length limits.
